...
## Corpus Check
- 197 files · ~133,075 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1510 nodes · 2046 edges · 133 communities (96 shown, 37 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 33 edges (avg confidence: 0.79)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b153d889`
...
Cohesion: 0.29
Nodes (7): eslint, jest, devDependencies, eslint, jest, tsconfig-paths, tsconfig-paths

### Community 1 - "ApiKeyService"
Cohesion: 0.25
Nodes (5): ApiKeyAuthGuard, extractCredential(), Injectable, ApiKeyService, Injectable
...
Cohesion: 0.07
Nodes (25): Cron, DAILY_PARTITION_NAME_PATTERN, DEFAULT_LOG_PARTITION_DAYS_AHEAD, DEFAULT_LOG_RETENTION_DAYS, LOG_RETENTION_LOCK_ID, LOG_RETENTION_LOCK_NAMESPACE, MAX_LOG_PARTITION_DAYS_AHEAD, MAX_LOG_RETENTION_DAYS (+17 more)

### Community 5 - "app.module.ts"
Cohesion: 0.05
Nodes (37): ApiServiceUnavailableResponse, Catch, AppController, Controller, Get, AppModule, Module, AppService (+29 more)
...
### Community 20 - "Q: Remove unused end-to-end test setup and standardize test environment filenames"
Cohesion: 0.40
Nodes (4): Answer, Outcome, Q: Remove unused end-to-end test setup and standardize test environment filenames, Source Nodes
...
### Community 28 - "log.mapper.ts"
Cohesion: 0.16
Nodes (12): CursorService, Injectable, LogResponseDto, QueryLogsResponseDto, ApiProperty, CursorPayload, RawLogRow, LogAttributeValue (+4 more)

### Community 29 - "tenancy.module.ts"
Cohesion: 0.14
Nodes (13): POSTGRES_UNIQUE_VIOLATION, TenantRefreshToken, Column, CreateDateColumn, Entity, Index, PrimaryGeneratedColumn, Tenant (+5 more)
...
### Community 32 - "tenant-auth.controller.ts"
Cohesion: 0.08
Nodes (32): ApiConflictResponse, HttpCode, TenantAuthController, ApiBadRequestResponse, ApiBody, ApiCreatedResponse, ApiOkResponse, ApiOperation (+24 more)

### Community 33 - "tenancy.constants.ts"
Cohesion: 0.20
Nodes (9): ACCESS_TOKEN_TTL_SECONDS, API_KEY_HEADER, API_KEY_PREFIX, DEFAULT_TENANT_ID, LOADGEN_TENANT_EMAIL, LOADGEN_TENANT_ID, LOADGEN_TENANT_PASSWORD, REFRESH_TOKEN_TTL_DAYS (+1 more)
...
Cohesion: 0.05
Nodes (39): A Note on AI Usage, Authentication and API Keys, Batch Behavior, CI Requirement, Configuration, Core Requirements, Credential Transport, Default Posture: Zero Configuration (+31 more)

### Community 40 - "ApiKey"
Cohesion: 0.14
Nodes (11): ApiKey, Check, Column, CreateDateColumn, Entity, Index, PrimaryGeneratedColumn, InjectRepository (+3 more)

### Community 42 - "logPulse"
Cohesion: 0.08
Nodes (26): API documentation, Attribute storage strategy, Backpressure, Bottlenecks discovered, Configuration, `GET /health`, `GET /logs/aggregate` — time-bucketed counts, `GET /logs` — query with combinable filters (+18 more)
...
### Community 63 - "logs.controller.ts"
Cohesion: 0.19
Nodes (10): getFirstIssueMessage(), parseWithSchema(), AggregateBucketDto, AggregateLogsResponseDto, ApiProperty, mapAggregationToResponse(), LogAggregationService, Injectable (+2 more)

### Community 64 - "TokenService"
Cohesion: 0.17
Nodes (6): RequestWithTenantId, extractBearerToken(), TenantJwtAuthGuard, Injectable, TokenService, Injectable
...
## Knowledge Gaps
- **707 isolated node(s):** `update-agent-context.sh script`, `$schema`, `collection`, `sourceRoot`, `deleteOutDir` (+702 more)
...
- **Why does `parseWithSchema()` connect `logs.controller.ts` to `tenant-auth.controller.ts`, `log-query.schema.ts`, `log-entry.validator.ts`, `.create`, `api-keys.controller.ts`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **Why does `Log` connect `aggregation-query.builder.ts` to `log.repository.ts`, `retention.service.ts`, `log-query.interface.ts`, `LogRepository`, `log.mapper.ts`?**
  _High betweenness centrality (0.007) - this node is a cross-community bridge._
- **Why does `LogRepository` connect `LogRepository` to `log.repository.ts`, `log-query.interface.ts`, `log-entry.validator.ts`, `log.mapper.ts`, `logs.controller.ts`?**
...
- **Should `retention.service.ts` be split into smaller, more focused modules?**
