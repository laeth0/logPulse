...
import {
  ...
  MAX_FUTURE_TIMESTAMP_OFFSET_MS,
...
const timestampSchema = z
  .string({ error: INVALID_TIMESTAMP_MESSAGE })
  .regex(ISO_8601_TIMESTAMP_PATTERN, { error: INVALID_TIMESTAMP_MESSAGE })
  .pipe(
    z.iso.datetime({
      offset: true,
      error: INVALID_TIMESTAMP_MESSAGE,
    }),
  )
  .refine(
    (timestamp) =>
