...
**Revised**: `password_hash` was originally seeded as `randomBytes(32).toString('hex')` stored directly (not bcrypt-hashed), on the grounds that this tenant only ever authenticates via its API key and never needs a working password. That made the account structurally un-loggable-into even by its own operator. It's now `passwordHasher.hash(LOADGEN_TENANT_PASSWORD)` — a real bcrypt hash of a configurable, insecure-by-default env var (mirroring `JWT_SECRET`/`DB_PASS`'s convention) — so an operator can log in as the load-generator tenant via the normal `POST /tenants/login` flow to inspect or manage its seeded key by hand. Like `LOADGEN_API_KEY` itself, this is only set on first seed (`ON CONFLICT (id) DO NOTHING`) — changing `LOADGEN_TENANT_PASSWORD` after the tenant row already exists has no effect until the row is manually reset.
