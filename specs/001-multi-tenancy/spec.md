...
- **Tenant**: A single customer account — not an organization with multiple users, teams, or roles. Owns its own log entries, its own API keys, and its own login credentials (email + password). Has an identity, an email, and a password credential. All log entries and API keys belong to exactly one Tenant, and a Tenant has no members other than itself.
