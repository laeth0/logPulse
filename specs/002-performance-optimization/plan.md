...
**Storage**: PostgreSQL 16 (existing, unchanged version). One new table: `log_rollups` (tenant-scoped, minute-granularity pre-aggregation). One existing column removed: `logs.attributes_text` (and its GIN index), superseded by type-branched containment queries against the already-existing `logs.attributes` column.
...
*Post-Phase-1 re-check*: PASS — see [research.md](./research.md)'s per-decision rationale; each decision that touches an existing hard requirement (rollup tenant-scoping, rollup/retention sync, non-blocking rebuild, durability) states explicitly how it's preserved, not just asserts it.
...
│   │   └── log.repository.ts                     # CHANGED: insertMany() gains the coalescing queue in front of copyLogsIn(); a rollup upsert follows each flush; findPage() switches to getRawMany()
│   └── services/
│       └── log-rollup-rebuild.service.ts          # NEW: OnApplicationBootstrap hook that rebuilds log_rollups from raw Log data if they're missing/stale — explicitly NOT awaited by the hook, so it never blocks app.listen()/GET /health (FR-019)
├── migrations/
│   ├── 1785684350114-CreateLogsTable.ts          # CHANGED (folded in, pre-release convention — see research.md Decision 11): attributes_text column removed
│   ├── <existing attributes_text GIN index migration>  # CHANGED: index-creation statement removed
│   └── <new-timestamp>-CreateLogRollupsTable.ts   # NEW: log_rollups table + its PK/index
└── retention/
    └── retention.service.ts                       # CHANGED: runMaintenance() gains a rollup-pruning/boundary-reconciliation step, reusing the existing advisory lock — no new lock, no new scheduled job
...
*No entries — no constitution violations to justify.* The one piece of genuinely new machinery (`log_rollups` and its rebuild/pruning logic) is directly required by spec.md FR-005/007/008/009 (traceable to measured evidence that raw-table aggregation cannot meet SC-003/SC-004 under load) — not introduced speculatively. Every other change (write coalescing, raw-row reads, dropping a redundant column, explicit pool sizing) reduces existing complexity/overhead rather than adding new abstraction.
