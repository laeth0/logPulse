...
5. **Given** the service restarts with pre-computed summaries missing or stale, **When** `GET /health` is polled immediately after, **Then** it reports ready without waiting for summaries to rebuild, and any aggregation requests issued during the rebuild period still return correct results via direct raw-table scan.
...
- What happens if the service restarts or crashes mid-way through building or updating a pre-computed aggregation summary? The summary must be recoverable/rebuildable from the durable log data itself — it must never become the only copy of any information, and must never cause `GET /logs/aggregate` to return incorrect counts after a restart. Per the Clarifications above, `GET /health` MUST NOT block on this rebuild — it reports ready as soon as its existing readiness conditions (database connected, migrations applied) are met, and aggregation transparently falls back to a direct raw-table scan (still correct, just slower) for any request that arrives while a rebuild is still in progress.
...
- **FR-009**: If pre-computed aggregation summaries are lost or become unavailable (e.g., after an unclean restart), the system MUST rebuild them from the durable log data in the background without blocking service startup (see FR-019), and MUST NEVER report an incorrect count in the meantime — aggregation requests arriving before the rebuild completes MUST be served correctly via direct raw-table scan, not from stale or partial summaries.
...
- **FR-019**: `GET /health` MUST NOT block on rebuilding pre-computed aggregation summaries after a restart — it MUST report ready as soon as its existing readiness conditions (database connectivity, applied migrations) are met, independent of rollup-rebuild status. `GET /logs/aggregate` MUST continue returning fully correct results during any period a rebuild is in progress, by falling back to a direct raw-table scan.
