...
- `003-ingestion-backpressure`: **spec → clarify → plan → tasks complete.** Spec has 18 FRs/8 SCs/3 user stories; clarified capacity signal (admitted-but-not-completed rows *and* estimated bytes, tracked globally, no CPU/event-loop/DB-utilization signals), validation-accepted-only counting, and the 413-vs-503 split (a batch that can *never* fit gets `413`, matching this project's existing `JSON_BODY_LIMIT` precedent; temporary exhaustion gets `503`+`Retry-After`). `tasks.md` (27 tasks, T001-T027) refined plan.md per new engineering constraints: HTTP-exception throwing moved OUT of `LogRepository` into a translation step in `LogIngestionService` (domain errors `IngestionCapacityExceededError`/`IngestionBatchTooLargeError` stay HTTP-agnostic), config parsing centralized into `createBackpressureConfig()`. US1/US2/US3 are sequential refinements of the same mechanism, not parallel tracks (US1 = bare 503/413 via built-in Nest exceptions = MVP; US2 = upgrade to Retry-After-carrying `BackpressureException`; US3 = verification-only). Full docs: `specs/003-ingestion-backpressure/{spec,plan,research,data-model,quickstart,tasks}.md` + `contracts/post-logs-backpressure.md`.

---

## 🚀 Next phase

**Goal:** Run `/speckit-implement` for `003-ingestion-backpressure` (or a specific phase of it) — still gated on user confirmation per this project's workflow; task generation explicitly stopped short of implementation this round, per instruction.
