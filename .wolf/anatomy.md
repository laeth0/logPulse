...
- `package-lock.json` — npm lock file (~112312 tok)
- `package.json` — Node.js package manifest (~955 tok)
- `projectSchema.dbml` (~1467 tok)
- `README.md` — Project documentation (~9303 tok)
...
- `unit_testing_implementation_plan.md` — Unit Testing Implementation Plan (~3987 tok)

## graphify-out/

- `.graphify_analysis.json` (~3389 tok)
- `.graphify_python` (~5 tok)
- `.graphify_root` (~14 tok)
- `.vocab.txt` — Declares types (~366 tok)
- `graph.json` (~59067 tok)
- `manifest.json` (~1639 tok)

## graphify-out/cache/

- `stat-index.json` (~2046 tok)

## graphify-out/cache/ast/v0.9.32/

- `42a7230cdc1aaf00f4a0a8c78bfa3fa46ca4b93a6eb4f28327722155638dc718.json` (~2716 tok)
- `46315faeec2a87b2ed4049fc128b6ff43f038a76097141721e2518640011e78f.json` (~790 tok)
- `6bace277a911f3dbc54ea989701f231f94b612ff5d77083b756a2159109eb60a.json` (~9836 tok)
- `79254ad5647d674ed8ea7513f76dd67c80d96d1ecb3abdf0686a1c39887a9fda.json` (~168 tok)
- `a289da2d045d96c4c551e39bcd85eeca5be69fe50bcbf600069df82ee2f3e748.json` (~585 tok)
...
- `log-api.constants.ts` — Exports DEFAULT_LOG_QUERY_LIMIT, MAX_LOG_QUERY_LIMIT, MAX_FUTURE_TIMESTAMP_OFFSET_MS, ATTRIBUTE_QUERY_PREFIX + 8 more (~349 tok)
...
- `global-exception.filter.ts` — Global exception filter that normalizes all thrown exceptions into the (~791 tok)
  - class `GlobalExceptionFilter` L25-71 (~465 tok)
  - section `ExternalClientError` L72-76 (~21 tok)
  - fn `getExternalClientError` L77-89 (~114 tok)
...
- `database.config.ts` — Builds a TypeORM {@link DataSourceOptions} object from environment variables. (~567 tok)
  - fn `createDatabaseOptions` L15-46 (~332 tok)
  - fn `createReadDatabaseOptions` L47-59 (~85 tok)
...
## src/logs/repositories/

- `log.repository.ts` — One caller's still-pending insertMany() call: its rows plus the settlers (~3294 tok)
  - section `PendingInsert` L39-45 (~59 tok)
  - section `RollupDelta` L46-54 (~38 tok)
  - class `LogRepository` L55-364 (~2836 tok)
...
- `app.e2e-spec.ts` — API routes: GET (1 endpoints) (~216 tok)
- `jest-e2e.json` (~75 tok)
